...
@section('content')
<div class="card-body">
    <form action="/edit-galeri/{{ $galeri->id }}" method="POST" enctype="multipart/form-data">
        @method('put')
        @csrf
    <div class="modal-body">
        <label class="form-label">Title Galeri</label>
        <div class="input-group input-group-outline mb-3">
            <input type="text" class="form-control" name="title" required value="{{ $galeri->title }}">
        </div>
        @error('title')
        ...
        @if ($galeri->image)    
            <img src="{{ asset('storage/' . $galeri->image) }}" class="img-preview mb-2" style="width: 30%; display:block">
        @else
